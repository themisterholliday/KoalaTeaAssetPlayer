...
public struct VideoAsset: AssetProtocol {
    ...
    public init(url: URL) {
        ...
        let timePoints = TimePoints(startTime: CMTime.zero, endTime: VideoAsset.adjustedTimeScaleDuration(for: urlAsset.duration))
    ...
    public func changeStartTime(to time: Double) -> VideoAsset {
        let cmTime = CMTimeMakeWithSeconds(time, preferredTimescale: PublicConstants.DefaultTimeScale)
    ...
    public func changeEndTime(to time: Double,
        ...
        guard cmTime < urlAssetDuration else {
            var offset = CMTime(seconds: 0, preferredTimescale: PublicConstants.DefaultTimeScale)
...
extension VideoAsset {
    ...
    private static func adjustedTimeScaleDuration(for duration: CMTime) -> CMTime {
...
extension VideoAsset {
    ...
    private static func getTimeRanges(for duration: Int, clipLength: Int) -> [CMTimeRange] {
        ...
        return result.compactMap { (value) -> CMTimeRange? in
            ...
            let start = CMTime(seconds: first.double - 1.0, preferredTimescale: PublicConstants.DefaultTimeScale)
            let end = CMTime(seconds: last.double, preferredTimescale: PublicConstants.DefaultTimeScale)
